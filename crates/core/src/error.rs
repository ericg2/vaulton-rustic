...
pub type RusticJobResult<T, E = RusticJobError> = Result<T, E>;
...
pub enum RusticJobError {
    #[error("The job has been cancelled.")]
    JobCancelled,

    #[error(transparent)]
    JobError(#[from] Box<RusticError>),
}

impl RusticJobError {
    pub fn display_log(&self) -> String {
        match self {
            RusticJobError::JobCancelled => "Job has been cancelled.".into(),
            RusticJobError::JobError(x) => x.display_log(),
        }
    }
}
