...
}, progress::{Progress, ProgressBars}, repofile::{
    IndexFile, IndexPack, PackHeader, PackHeaderLength, PackHeaderRef, packfile::PackId,
}, repository::{Open, Repository}, JobCancelToken, RusticJobResult, RusticJobError};
...
pub(crate) fn check_repository<P: ProgressBars, S: Open>(
    ...
    if !opts.trust_cache {
        if let Some(cache) = &cache {
            for file_type in [FileType::Snapshot, FileType::Index] {
                ...
                check_cache_files(20, cache, raw_be, file_type, &p, &collector, &token)?;
            }
        }
    }
    ...
    if let Some(hot_be) = hot_be {
        for file_type in [FileType::Snapshot, FileType::Index] {
            check_hot_files(raw_be, hot_be, file_type, pb, &collector, &token)?;
        }
    }

    let (index_collector, missing_packs) = check_packs(be, hot_be.as_ref(), pb, &collector, &token)?;
    ...
    if let Some(cache) = &cache {
        ...
        if !opts.trust_cache {
            ...
            check_cache_files(5, cache, raw_be, FileType::Pack, &p, &collector, &token)?;
        }
    }

    let index_be = GlobalIndex::new_from_index(index_collector.into_index());

    let packs = check_trees(be, &index_be, trees, pb, &collector, &token)
        .map_err(|err| {
            if let RusticJobError::JobError(err) = err {
                collector.add_error(CheckError::ErrorCheckingTrees { source: err });
            }
        })
        .unwrap_or_default();
    ...
    if opts.read_data {
        ...
        token.ensure_check()?; // *** do a check before expensive warmup!
...
fn check_hot_files(
    ...
    for (id, size_hot) in files_hot {
        match files.remove(&id) {
            ...
            _ => {} //everything ok. no need to add token here... too late!
        }
    }
...
fn check_cache_files(
    ...
    files
        .into_par_iter()
        .try_for_each_with((cache, be, p), |(cache, be, p), (id, size)| {
...
fn check_packs(
    ...
    collector: &CheckResultsCollector,
    token: &JobCancelToken,
) -> RusticJobResult<(IndexCollector, BTreeMap<PackId, u32>)> {
...
fn check_trees(
    ...
    collector: &CheckResultsCollector,
    token: &JobCancelToken,
) -> RusticJobResult<BTreeSet<PackId>> {
    token.ensure_check()?;
