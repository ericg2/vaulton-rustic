...
use rustic_core::{BackupOptions, CommandInput, ParentOptions, PathList, SnapshotGroupCriterion, SnapshotOptions, StringList, repofile::{PackId, SnapshotFile}, JobCancelToken};
...
fn test_backup_with_tar_gz_passes(
    ...
    // first backup
    let first_snapshot = repo.backup(&opts, paths, SnapshotFile::default(), JobCancelToken::new())?;
    ...
    let repo = repo.to_indexed_ids()?;
    // second backup
    let second_snapshot = repo.backup(&opts, paths, SnapshotFile::default(), JobCancelToken::new())?;
    ...
    let third_snapshot = repo.backup(&opts, paths, snap, JobCancelToken::new())?;
...
fn test_backup_dry_run_with_tar_gz_passes(
    ...
    // dry-run backup
    let snap_dry_run = repo.backup(&opts, paths, SnapshotFile::default(), JobCancelToken::new())?;
...
fn test_backup_stdin_command(
    ...
    let snapshot = repo.backup(&opts, &paths, SnapshotFile::default(), JobCancelToken::new())?;
    insta_snapshotfile_redaction.bind(|| {
