...
use rustic_core::{DataBackends, ErrorKind, IndexedStatus, NoProgressBars, RepoIndexed, Repository, RepositoryBackends, RusticError, RusticResult, WriteBackend};

use crate::{local::LocalBackend, util::{BackendLocation, location_to_type_and_path}, RepoVfsBackend};
...
#[cfg(feature = "clap")]
use clap::ValueHint;
use crate::services::PointSource;
...
pub trait PointResolver: Send + Sync {
    fn get_repo(&self, src: &PointSource) -> RusticResult<Arc<RepoIndexed>>;
    fn get_repo_vfs(&self, src: &PointSource) -> RusticResult<Arc<RepoVfsBackend>>;
    fn get_data(&self, src: &PointSource) -> RusticResult<Arc<DataBackends>>;
}
