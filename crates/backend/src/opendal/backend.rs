...
use log::{error, trace};
use opendal::blocking::Lister;
use opendal::blocking::Reader;
use opendal::options::WriteOptions;
use opendal::{
    blocking::Operator, layers::{ConcurrentLimitLayer, LoggingLayer, RetryLayer, ThrottleLayer}, options::{ListOptions, ReadOptions}, Entry,
    EntryMode,
    Metadata,
    Scheme,
};
...
use std::{collections::BTreeMap, io, str::FromStr, sync::OnceLock};
...
use rustic_core::{
    DataLocation, ErrorKind, FileType, Id, ReadBackend, RusticError, RusticResult, WriteBackend, ALL_FILE_TYPES,
};
...
impl OpenDALBackend {
    ...
    pub fn new(path: impl AsRef<str>, options: BTreeMap<String, String>) -> RusticResult<Self> {
        ...
        Ok(Self { id: Uuid::new_v4(), operator })
    }
