...
use rustic_core::{JobCancelToken, LocalDestination, LsOptions, Repository, RepositoryOptions, RestoreOptions};
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let restore_infos = repo.prepare_restore(&opts, ls.clone(), &dest, dry_run, token.clone())?;
    repo.restore(restore_infos, &opts, ls, &dest, token)?;
    Ok(())
}
